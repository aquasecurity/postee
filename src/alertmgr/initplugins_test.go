...
func TestBuildJiraPlugin(t *testing.T) {
	...
	for _, test := range tests {
		...
		if r.Url != test.jira.Url { t.Errorf("Wrong URL:\nWaited: %q\nResult: %q", test.jira.Url, r.Url)}
		if r.User != test.jira.User { t.Errorf("Wrong User:\nWaited: %q\nResult: %q", test.jira.User, r.User)}
		if r.Password != test.jira.Password { t.Errorf("Wrong Password:\nWaited: %q\nResult: %q", test.jira.Password, r.Password)}
		if r.ProjectKey != test.jira.ProjectKey { t.Errorf("Wrong ProjectKey:\nWaited: %q\nResult: %q", test.jira.ProjectKey, r.ProjectKey)}
		if r.Issuetype != test.jira.Issuetype { t.Errorf("Wrong Issuetype:\nWaited: %q\nResult: %q", test.jira.Issuetype, r.Issuetype)}
		if r.Priority != test.jira.Priority { t.Errorf("Wrong Priority:\nWaited: %q\nResult: %q", test.jira.Priority, r.Priority)}
		if r.Assignee != test.jira.Assignee { t.Errorf("Wrong Assignee:\nWaited: %q\nResult: %q", test.jira.Assignee, r.Assignee)}
	}
}
